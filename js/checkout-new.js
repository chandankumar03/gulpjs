	...
	(function( z, $ ) {
		...
		var o = {
			...
			init : function() {
					...
					switch( _id ) {
						case 'bill_country_id':
							...
							if( _country != 'in' ) {
								...
								$( '#bill_region_txtbox' ).show().focus();
							} else {
								$( '#bill_region_txtbox' ).val( '' ).hide();
					...
					if( e.which == $.ui.keyCode.ENTER ) {
						switch( $( this ).attr( 'class' ).toLowerCase() ) {
							case 'quntity-input':
								...
								break;
							case 'inputcoupon':
								if( $( '#apply_coupon' ).is( ':visible' ) && ( o.isCouponApplied == 0 ) ) {
									o.isCouponApplied = 1;
				...
				$( '#page' ).on( 'click', 'a#assembly_product_count, a#form_assembly_product_count, a#okay_got_it, a#form_okay_got_it, a#empty_user_cart, a#scroll_to_cart, a#form_remove_non_serviceable_items, a#remove_non_serviceable_items, #shpt, #shptFinalPage', function(e) {
					var _id = $( this ).attr( 'id' ).toLowerCase();
							...
							*/
							...
							if( input_type == 'emi' ) {
								...
								if( parseInt( value ) == 0 ) {
									...
									$(this).parent('.pf-radio-big-inline').click();
								}
			...
			handleDeleteCartProduct : function( _data, _params ) { 
				...
				var oosLen = $( '#notify_form_wrap_' + _params.pid ).length;
                                /***
				...
				if(  oosLen == 0 ) {
					...
					o.timedObjects[ data.removed_item ] = setTimeout( function() {
						if( $( '.order-summary-row-wrapper' ).length == 1 ) {
					...
					o.deleteTimedObject( _params.pid, data.removed_item );
					$( '#cartitem_' + _params.pid ).parent( '.order-summary-row-wrapper' ).children( '.out-of-stock-box' ).hide().removeClass( 'out-of-stock-box' );
				}
				...
				//Added code for gift card phase - 2
				if(giftcount > 0){
					var isGiftCard = $('#giftcard_'+_params.pid).val();
					var giftCardNature = $('#giftcard_nature_'+_params.pid).val().toLowerCase();
					if(isGiftCard == 1){
			...
			handleUndoDeleteCartProduct : function( data, _params ) {
				...
				var physicalGiftCount = temp_data.physical_giftcard_count;
				var coupon = temp_data.coupon;
				...
				var giftCardNature = $('#giftcard_nature_'+_params.id).val().toLowerCase();
				if(giftCardNature == 'physical'){					
					$('input#physical_giftcard_count').val(physicalGiftCount);
				}
				...
				if( o.pageType == 'AddressSelection' && $(".order-summary-row-wrapper .delivery-details  .checkout-message").is(":visible") ) {
					$( '.btn_green_big' ).addClass( 'disabled' );
					$( '.btn_green' ).addClass( 'disabled' );
				}
			...
			setPricing : function( _data, id ) {
				...
				if( typeof data.online_shipping_charge != "undefined" && $( "#shipping_handling" ).length > 0 ) {
					...
					if(data.online_shipping_charge == 0 ) {
						if( !$( '#shipping_handling .price-dtl .txt-red' ).is( ':visible' ) ) {
                                                        $( '#shipping_handling .price-dtl .txt-red' ).show();
						...
						$( '#shipping_handling .price-dtl .txt-red' ).hide();
						$( '#shipping_handling .price-nmbr' ).html( 'Rs. ' + data.online_shipping_charge );
					}
				}
						...
						if(typeof $( '#ckShippingAddress input[name="country_id"]' ).val() != 'undefined')
							{
								...
								if(cnt !="india"){
										$("#serviceablity_error_msg").html("Some Of Your Products are not available for international shipping.").show();
										$("#serviceablity_error_msg").css("margin","15px");
								}
							}
			...
			handleAssemblyResponse : function( _data, _params ) {
				...
				if( typeof data.data != 'undefined' ) {
					...
					var isPartPay = parseInt( data.data.is_partpay);
					var delivery_info = data.data.delivery_info;
                                        var is_cod_exists = data.data.cod;
					...
					/* Added by prathamesh for pincode java api response */
					if(_params.cityId != '' && _params.regionId != '') {
						$( '#' + _params.cityId ).val( '' );
						...
						if( data.data.city != '' &&  data.data.region != '') {
							$( "#" + _params.cityId ).val( data.data.city );
							$( "#" + _params.cityId ).parent().addClass( 'input-filled' );
	
							/*
							if( ( typeof data.state == 'undefined' ) || ( data.state == '' ) ) {
								data.data.state = $( '#' + _params.regionId + ' option:first' ).val();
							}
							*/
							// add state only if it's valid
							if( $( '#' + _params.regionId + ' option[value="' + data.data.state + '"]' ).length > 0 ) {
								$( '#' + _params.regionId ).select2( 'val', data.data.state );
							} else {
								$( '#' + _params.regionId ).select2( 'val', $( '#' + _params.regionId + ' option:first' ).val() );
							}
	
							var locality = $.parseJSON( data.data.locality );
					...
					/* Code added by prathamesh for pincode java api response*/

					if( _serviceableCheck === 0 ) {
						if( $.inArray( o.pageType, [ 'OrderSummary', 'AddressSelection', 'FirstTime' ] ) != -1 )  {
							...
							// only on address selection
							if( ( o.non_serviceable_items.length > 0 ) && ( o.pageType == 'AddressSelection' ) ) {
								$( '.btn_green_big' ).addClass( 'disabled' );
								$( '.btn_blue' ).addClass( 'disabled' );
								$( '.btn_green' ).addClass( 'disabled' );
							} else {
							...
							$( 'div[id^="cartitem_"]' ).each(function() {
								...
								if( $.inArray( _id, o.non_serviceable_items ) == -1 ) {
									$( 'div[id^="cartitem_' + _id + '"] .checkout-message.error' ).hide();
									$( 'div[id^="is_serviceable_info_' + _id + '"]' ).show();
									$( 'div[id^="is_serviceable_info_' + _id + '"] p' ).show();
								}

                                                                if( is_cod_exists == false ) {
                                                                    $( 'div[id^="cartitem_' + _id + '"] .cod_msg' ).html( 'Not Available' );
                                                                } else if( $( 'div[id^="cartitem_' + _id + '"] .cod_msg' ).data( 'cod-flag' ) == 1) {
                                                                    $( 'div[id^="cartitem_' + _id + '"] .cod_msg' ).html( 'Available' );
                                                                }
							});
					...
					try {
						if( productsWithAssemblyError.length == 0 ) {
							$( 'p[id^="assembly_status_"]' ).each(function() {
								var _node = $( this );
							...
							for( var i = 0; i < productsWithAssemblyError.length; i++ ) {
								var _node = $( 'p[id="assembly_status_' + productsWithAssemblyError[ i ] + '"]' );
					...
					if( typeof delivery_info != 'undefined' ) {
						for( var i in delivery_info ) {
							if( delivery_info.hasOwnProperty( i ) ) {
								if ( delivery_info[ i ].tentative_delivery_date.start_day != '' && delivery_info[ i ].tentative_delivery_date.start_day !==  null && delivery_info[ i ].tentative_delivery_date.end_day != '' && delivery_info[ i ].tentative_delivery_date.end_day !==  null ) {
									...
									$( '#p_delivery_date_'+i ).hide();
                                         ...
                                         * IF FREE GIFT ERROR SHOW IS TRUE THE CUSTOMER CAN'T PROCEED FUTHER
                                         */
					if( proceed && $( '.order-summary-row' ).length && $( '.out-of-stock-box' ).length == 0 ) {
						$( '.btn_blue' ).removeClass( 'disabled' );
                                                if(!isFreeGiftErrorshow) {
			...
			updateAssemblyInfoOfNodes : function(_id , _node , assembly_info){
				...
				//Added by prathamesh for pincode java api response
				if(assembly_info[_id]['assembly'] == 'To be arranged by yourself' ) {
			...
			/*setAssemblyMessaging : function( _id, productsWithAssemblyError, _node ) {
				var assemblyType = $.trim( _node.attr( 'data-assembly' ).toLowerCase() );
				var assemblyBrand = $.trim( _node.attr( 'data-assembly-brand' ) );

				if( ( $.inArray( assemblyType, [ 'assembly by pepperfry', 'carpenter assembly' ] ) >= 0 ) && ( $.inArray( _id, productsWithAssemblyError ) < 0 ) ) {
					_node.find( 'b' ).removeAttr( 'class' ).addClass( 'txt-green' ).html( 'Provided by Pepperfry' );
				} else if( assemblyType == 'assembly by brand' && ( $.inArray( _id, productsWithAssemblyError ) < 0 ) ) {
					_node.find( 'b' ).removeAttr( 'class' ).addClass( 'txt-green' ).html( 'Provided by ' + assemblyBrand );
				} else if( assemblyType == 'self assembly' && ( $.inArray( _id, productsWithAssemblyError ) < 0 ) ) {
					_node.find( 'b' ).removeAttr( 'class' ).addClass( 'txt-green' ).html( 'Simple Assembly, Can be managed by Self' );
				} else if( $.inArray( _id, productsWithAssemblyError ) >= 0 ) {
					_node.find( 'b' ).removeAttr( 'class' ).addClass( 'txt-red' ).html( 'To be Arranged by Yourself' );
				} else if( assemblyType == 'no assembly required' ) {
								...
								$.each(promo_skus ,function(key,value){
									...
									if(typeof delivery_info != 'undefined'){
									    if (delivery_info.tentative_delivery_date.start_day != '' && delivery_info.tentative_delivery_date.start_day !==  null && delivery_info.tentative_delivery_date.end_day != '' && delivery_info.tentative_delivery_date.end_day !==  null){
									        ...
									        str = str +'<p id="p_delivery_date_'+value+'">Delivery: <span class="txt-green status" id="delivery_date_cart_'+value+'">'+ start_day +' - '+ end_day +'</span></p><p>COD: <b class="txt-red cod_msg" data-cod-flag="0">Not Available</b></p>';
									    }
									}
									str = str +'</div></div></div></div><div class="pricing-details"><div class="display-tbl"><div class="table-cell"><div class="strike_txt"></div><div class="final_pricing "><span class="txt-red"> Free!</span></div></div></div></div><div class="trash-order-product"><div class="display-tbl"><div class="table-cell"></div></div></div></div>';
									...
									$('.order-summary-row-wrapper:last-child').append(str);
									if($('.btn_green').hasClass('disabled')){
										$('.btn_green').removeClass('disabled')
									}
									if($('.btn_green_big').hasClass('disabled')){
						...
						case 'cancel': 
							...
							if(data.free_gift){ 
								...
								$.each( removed_items, function( key, val ) {
									$( '#cartitem_' + val ).siblings( '.delete_item' ).attr( 'data-hash', data.removed_item );
									...
									$( '#cartitem_' + val ).parent( '.order-summary-row-wrapper' ).children( '.out-of-stock-box' ).hide().removeClass( 'out-of-stock-box' );
			...
			setDefaultAddress : function( _id, _type ) {
				...
				el.find( 'input[type="hidden"][name="mobile"]' ).val( _address.mobile );
                                ...
                                if(elem == 'ckShippingAddress' && utils.readCookie( 'billing_address_changed' ) != 1) {
                                    $('#bill_addressBook'+_id).prop('checked', true);
                                    o.setBillingAsShipping( _id, _type );
                                } else if(elem == 'ckShippingAddress' && utils.readCookie( 'billing_address_changed' ) == 1) {
			...
			showAddressForEdit : function( _id ) {
				...
				$( '#' + fieldPrefix + 'region_txtbox' ).val( _address.region_text ).parent().addClass( 'input-filled' );
				...
				if( $.trim( _address.country.toLowerCase() ) != 'india' ) {
					$( '#s2id_' + fieldPrefix + 'region_select' ).select2( 'val', ' ' );
					//$( '#s2id_' + fieldPrefix + 'region_select' ).select2( 'disable' );
					$( '#s2id_' + fieldPrefix + 'region_select' ).css('display','none');
					$( '#' + fieldPrefix + 'region_txtbox' ).val( _address.state ).show();
					$( '#' + fieldPrefix + 'region_txtbox' ).next().css('display','block');
				} else {
					$( '#s2id_' + fieldPrefix + 'region_select' ).select2( 'val', _address.state.replace( /&amp;/g, '&' ) );
					$( '#' + fieldPrefix + 'region_txtbox' ).val( '' ).hide();
					$( '#s2id_' + fieldPrefix + 'region_select' ).css('display','inline-block');
					$( '#' + fieldPrefix + 'region_txtbox' ).next().css('display','none');
				}



				// do a exact country match
				var selectedCountry = $( '#' + fieldPrefix + 'country_id  option' ).filter(function() {
					return $( this ).html().toLowerCase() == _address.country.toLowerCase()
				}).val();

				selectedCountry = $.trim( selectedCountry );
				if( typeof selectedCountry == 'undefined' || selectedCountry == '' || selectedCountry == false ) {
					selectedCountry = $( '#' + fieldPrefix + 'country_id  option:eq(0)' ).val();
				}
			...
			loginValidation : function( _id ) {
				...
				var stateTextBox = $( '#' + prefix + 'region_txtbox' );
				...
				if( country.toLowerCase() == 'in' ) {
					var _pin = $.trim( postcode.val() );

					if( ( _pin == '' ) || !$.isNumeric( _pin ) || ( _pin.length != 6 ) ) {
						// $( '#s2id_' + prefix + 'region_select' ).val( ' ' );
						// $( '#' + prefix + 'region_select' ).parent().siblings( '.error-text' ).show();
						$( '#' + prefix + 'postcode' ).parent().siblings( '.error-text' ).show();
						$( '#' + prefix + 'postcode' ).val( '' );
						isFormValid = false;
					}
				} else if( country != '' ) {
					...
					$( '#s2id_' + prefix + 'country_id' ).select2( 'val', $( '#' + prefix + 'country_id option:eq(0)' ).val() );
					$( '#' + prefix + 'country_id' ).siblings( '.error-text' ).show();
				}

				if( $.trim( state.val() ) == '' ) {
					if( country == 'IN' ) {
						$( '#' + prefix + 'region_select' ).val( ' ' );
						$( '#' + prefix + 'region_select' ).parent().siblings( '.error-text' ).show();
						isFormValid = false;
					} else if( $.trim( stateTextBox.val() ) == '' ) {
						$( '#' + prefix + 'region_select' ).val( ' ' );
						$( '#' + prefix + 'region_select' ).parent().siblings( '.error-text' ).show();
						isFormValid = false;
					}
				...
				if( typeof data.data.success != "undefined" && data.data.success == true ) {
					...
					if( address_error == "success" ) {
						...
						if(typeof address_error.save_error != "undefined") {
							if(address_error.save_error == 'CSRF Token expired.') {
								o.changeButtonState( 'original' );
								...
								$("#serviceablity_error_msg").html(address_error.header).show();
...
//                              
			...
			handleDeleteItemResponse : function( data, _params ) {
                            ...
                            if(itemData.length > 0    ){
                                ...
                                for( var i in itemData ) {
                                        if( itemData.hasOwnProperty( i ) ) {
                                            var items = itemData[ i ];
                                            if( items.id == _params.product_id ) {
                                                    cartItemData = items;
                                                    break;
                                            }
                                        }
			...
			handleCartBehaviour : function( data, product_id, type ) {
				...
				if( typeof data.data.status != "undefined" && data.data.status == "success" ) {
                    ...
                    if( $( "#address_select_form" ).length > 0 && $(".order-summary-row-wrapper .delivery-details  .checkout-message").is(":visible") ) {
                        isProceed = 0;
                    }
                    ...
                    // if( ( data.total_amount == 0 ) && ( $( '.order-summary-row-wrapper .delete_item' ).length == 0 ) ) {
			...
			removeNonServiceableItem : function() {
				$('.cart-loader').show();
				if( $( ".order-summary-row" ).length == o.non_serviceable_items.length ) {
					o.non_serviceable_items.length = 0;
					o.emptyCart();
				} else {
			...
			giftcard : {
				...
				handleGiftCardResponse : function( data ){
					...
					if(response.status && response.amount > 0){
						...
						var html = '<div class="gv-amt-row clearfix"><div class="gv-amt-row-left"><div class="gv-amt-row-left-inner"><p>(-) Pepperfry Gift Card <a href="javascript:void(0)" data-card_number="'+$('.input-gv-number').val()+'" class="gv_rmv_voucher">Remove</a></p><span class="gv-amt-left-subttl">'+response.card+'</span></div></div><div class="gv-amt-row-right"><p>'+utils.currencyFormat( Math.ceil( response.amount ) )+'</p></div></div>';
						$('.gv-amt-wrap').append(html);
				...
				remove_giftvocher : function(){
					...
					if(typeof PF.CHECKOUT.giftcard.giftcard_applied!=='undefined' && _pos!==-1){
						$('.fail').hide();
						$('.success').hide();
						$(this).closest('.gv-amt-row').slideUp(function () {
	                        $(this).remove();

							if( $('.gv_rmv_voucher').length < 5 ) {
								$('.gv-apply-wrap').show();
								$('#payGiftSubttl').show();
								$('#payGiftMaxSubttl').hide();
							}
			...
			handleGrowTree : function( status ) {
				...
				if( typeof status != 'undefined' && status == 'checked' ) {
					...
					if($(".pricing-gray-box").length == 0){
						$( '#total_pay_coupon1' ).html( 'Rs. ' + finalyoupay );
					}
					else if($(".pricing-gray-box").length > 0){
			...
			handleWalletError : function() {
				...
				if( $( '#payment_method' ).length > 0 ) {
					...
					if( totalPay > parseInt( wallet_price_check ) ) {
						...
						$( '#' + prepend + 'checkoutWALLETContainer .ck-wallet-unavailable' ).show();
					} else {
						$( '.wallet_err' ).hide();
						...
						$( '#' + prepend + 'checkoutWALLETContainer .ck-wallet-unavailable' ).hide();
					}
				}
			},
			...
			feCheckBoxChange : function() {
				...
				if (this.checked) {
					...
					utils.eraseCookie('furnitureExchange');
                        ...
                        OOSNotification: function ( itemId ) {
                            var email_id_from = $( '#notify_email_' + itemId ).val();
                            var filter = /^[a-zA-Z0-9._-]+@([0-9a-z][0-9a-z.-]+\.)+[a-zA-Z]{2,4}$/i;
                            var emailValue = $.trim( email_id_from );

                            if ( emailValue == "" ) {

                                $('#notify_form_wrap_' + itemId + ' .error-text').show().empty().html( 'Enter your Email' );

                            }
                            else if ( filter.test( emailValue ) != true )
                            {
                                $( '#notify_email_' + itemId ).val( '' ).focus();
                                ...
                                var data = { 'email': emailValue, 'pid': itemId };
                                var url  = root_url + '/site_product/oos_notifciation_request';
                                var _params = {
					'itemId' : itemId
				};
                        ...
                        oosResponse: function ( data , _params ) {
                            if( data == "Success" ) {
                                    var itemLen = $( '.order-summary-row-wrapper' ).length;
                                    if( itemLen > 1 ) {
                                        $("#cartitem_" + _params.itemId + " .trash").click();
                                    } else {
                                        o.emptyCart();
                                    }
                            } else {
