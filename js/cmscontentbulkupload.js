...
(function() {
	...
	$("#save-data").click(function(){
               console.log(hit);
		if(hit==1) {
			var $url = root_url + "/product_cms/save_data";
			if($(this).parents('.pincode_upload').length){
				$url = root_url + "/config_pincode/save";
				var $data = "1=1&token="+$.trim($("#token").val())+"&location="+$.trim($("#location").val())+"&is_update="+$("#is_update").val(); 
			}else{
				var $data = "1=1&token="+$.trim($("#token").val())+"&location="+$.trim($("#location").val());
            }            //alert($data);
			$.ajax({
				type: "POST",
				url: $url,
				data: $data,
				 beforeSend: function() {
				    ...
				    $("#save-data").attr("disabled", true);
                                    $("#behaviour").attr("disabled", true);
                                    $("#save-data").val('Please wait while the data is uploaded. Do not hit the ‘Refresh’ or ‘Back’ button.');
                                    timer = setTimeout('getProgress()',5000);
				},
