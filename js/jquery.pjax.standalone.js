...
(function(){
		...
		internal.request(options.url, function(html){
			...
			if(typeof options.title == 'undefined'){
				...
				if(options.container.getElementsByTagName('title').length != 0){
					...
					options.container.getElementsByTagName('title')[0].outerHTML = '';
				}else{
	...
	internal.request = function(location, callback){
		...
		try {xmlhttp = window.XMLHttpRequest?new XMLHttpRequest(): new ActiveXObject("Microsoft.XMLHTTP");}  catch (e) { }
			...
			//location = replaceQueryString(location, '_pjax', '');
			
			old_qry = location.substr(location.indexOf('?'), location.length);
