var timer = 0;
var legalActions = {"enable":"Enable", "disable":"Disbale", "delete": "Delete", "alive": "Run Make Live","assign_category":"Assign Category","remove_category":"Remove Category","make_oos":"Out of Stock","make_is":"In Stock","mimport":"Run Media Import","af_value":"Update AF Value"};
		...
		if(val.length > 0) {
			if(confirm("Are you sure you want to perform " +  $("#drpAction").val() + " action?")) {
				if(confirm("Pakka?")) {
					...
					$.ajax({
						type: 'POST',
						url: ROOT_URL +"/merchant_listing/actions/",
						data:query_str,
						beforeSend: function() {
							$("#message").html("");
...
$(document).ready(function(){
	...
	$('#btnSubmit').click(function(e){
		if($.trim($("#drpAction").val()) == "") {
			...
			if($.trim($("#drpAction").val()) == "af_value" && $.trim($("#txt-af_value").val()) != "") {
...
var url = ROOT_URL + '/merchant_listing/filter_products/' + SUPPLIER_ID;
