...
$(document).ready(function () {
    ...
    $('.navigation ul li.level-top').hover(function () {
        ...
        if (curr_menu.find('.megamenu').length > 0) {
            mmenu_tout = setTimeout(function () {
                curr_menu.find('.megamenu').fadeIn(300);
                $('.transoverlay').show();
    ...
    $('.transoverlay').hover(function () {
        clearTimeout(mmenu_tout);
        mmenu_tout = setTimeout(function () {
            $('.megamenu,.transoverlay').fadeOut(300);
            $('.navigation ul li.level-top').removeClass('curr_menu_item');
        }, 300);
    });
            ...
            else
            {
                ...
                if (!redirect_callback) {
                    try {
                        ...
                        alert('some issue with response');
                    }
                }
                var cartItemData = {};
                if (page_type == 'vip')
                ...
                if (redirect_callback != '') {
                    ...
                    if (!$('#mini_cart').hasClass('active')) {
                        ...
                        $('.minicart_tab_header span').trigger('click');
                        $('#minicart_tab_panel_1 .wrapper').animate({
                            scrollTop: $('#mini-cart-card-' + product_id).offset().top
                        }, 'slow');
                    }
                }
...
$('.quick_gal_img').click(function (e) {
    e.preventDefault();
    $('#loading_img').show();
    $('#quick_large').attr('src', $(this).attr('data-large')).load(function () {
        $('#loading_img').hide();
    });
});
    ...
    if (!ajaxLoading) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.assembly) {
                    var assembly_vid = '';
                    var assembly_not_provided_msg = '<a href="javascript://" class=\"tooltip\" title=\"We\'re unable to provide assembly services for this pincode. You\'ll need to make offline arrangements for this item.\">Assembly required,<br>to be arranged by yourself</a>';
                ...
                if (data.additional_requirement) {
                    if (data.additional_requirement.template != 'not required') {
                        $('.additional_vip_notification').fadeIn(200).html('Customers in <strong>' + data.additional_requirement.state.toUpperCase() + '</strong> are required to submit additional documents. <a href="javascript://" class="pincode_rules"><u>Learn More</u></a>');
                        $('#vip_add_popup').html('');
            ...
            if (typeof data.data.success != "undefined" && data.data.success == true) {
                ...
                if (typeof data.data.online_shipping_charges != "undefined" && $("#shipping_handling").length > 0) {
                    ...
                    if (data.data.online_shipping_charges == 0) {
                        $("#shipping_handling .p_right").html("<span class='red'>(Free)</span>Rs. " + data.data.online_shipping_charges);
                    } else {
                ...
                $.each(cart, function (j, val) {
                    if ($("#cartitem_" + j).length > 0)
                    {
                        ...
                        {
                            unit_price = parseInt($("#cartitem_" + j).find(".disp_table_cell .text_1:last").data("price"));
                        }
                ...
                console.log(item_total + " = " + extra_discount + " = " + additional_discount + " = " + shipping_charge);
                total_pay = item_total + total_tax - extra_discount - additional_discount + shipping_charge;
