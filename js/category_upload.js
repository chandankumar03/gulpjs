...
(function() {
	...
	console.log(hit);
		if(hit==1) {
                        // Below line added to check for which bulkupload is called 
                        var upload_for = $("input[name=upload_for]:checked").val();
			var $url = root_url + "/category_bulkupload/save";
			var $data = "action=" + $("#behaviour").val() + "&1=1&token="+$.trim($("#token").val())+"&upload_for="+upload_for;
			$.ajax({
				type: "POST",
				url: $url,
				data: $data,
